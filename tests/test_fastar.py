...
class FastarTest(jtu.JaxTestCase):
  @parameterized.named_parameters(
    jtu.cases_from_list({'testcase_name': str(op), 'op': op}
                        for op in (
                            lax.abs, lax.ceil, lax.cos, lax.sin, lax.exp,
                            lax.expm1, lax.floor, lax.neg, lax.sign, lax.tanh,
                            special.expit)))
  ...
  @parameterized.named_parameters(
    jtu.cases_from_list(
      {'testcase_name': str(op), 'op': op} for op in (lax.log, lax.log1p)))
  def test_unop_positive_arg(self, op):
    check_custom_input(op, lambda rng: np.abs(rng.randn(1, 2)))

  @parameterized.named_parameters(
    jtu.cases_from_list({'testcase_name': str(dtype), 'new_dtype': dtype}
  ...
  @parameterized.named_parameters(
    jtu.cases_from_list({'testcase_name': str(op), 'op': op}
                        for op in (
                            lax.add, lax.div, lax.eq, lax.ge, lax.gt, lax.le,
                            lax.lt, lax.max, lax.min, lax.mul, lax.ne, lax.rem,
                            lax.sub)))
  ...
  def test_add_scalar_scalar_as_numpy(self):
    check_custom_input(lax.add, lambda rng: (
      np.float64(rng.randn()), np.float64(rng.randn())))

  def test_add_scalar_scalar_as_array(self):
    check_custom_input(lax.add, lambda rng: (
      np.array(rng.randn()), np.array(rng.randn())))
  ...
  @parameterized.named_parameters(
    {'testcase_name': "axes={}_shape={}".format(axes, shape),
  ...
  @parameterized.named_parameters(
    {'testcase_name': str(shapes), 'shapes': shapes}
    for shapes in (((2,), (2,)),
                   ((3, 2), (2,)),
                   ((2,), (2, 3)),
                   ((3, 2), (2, 4))))
  ...
  def test_dot_general_tensor_matrix(self):
    check(lambda x, y: lax.dot_general(x, y, dimension_numbers=(
      ((2,), (1,)), ((0,), (0,)))), (1, 2, 3), (1, 3))
  ...
  def test_select(self):
    check_custom_input(lax.select, lambda rng: (
      rng.rand(3, 2) > 0.5, rng.randn(3, 2), rng.randn(3, 2)))

  def test_select_scalar_pred(self):
    check_custom_input(lax.select, lambda rng: (
      rng.rand() > 0.5, rng.randn(3, 2), rng.randn(3, 2)))
  ...
  def test_concatenate_2(self):
    check(lambda x, y: lax.concatenate((x, y), dimension=2), (1, 2, 1),
          (1, 2, 3))

  def test_concatenate_3(self):
    check(lambda x, y, z: lax.concatenate((x, y, z), dimension=1), (2, 1),
          (2, 3), (2, 2))

  def test_slice(self):
    check(
      lambda x: lax.slice(x, start_indices=(1, 1, 0), limit_indices=(2, 2, 1),
                          strides=(2, 1, 1)), (7, 2, 1))
  ...
  def test_convolution(self, strides, padding, lhs_dilation, dimension_numbers,
                       lhs_perm, rhs_perm):
    lhs_shape = (1, 2, 4, 4)
    rhs_shape = (3, 2, 3, 2)
    check(
